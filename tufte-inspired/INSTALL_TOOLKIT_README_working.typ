...
#let sidenote(dy: 0pt, content) = marginalia.note(
  numbering: (.., i) => super[#i],
...
#let marginnote(dy: 0pt, content) = marginalia.note(
  numbering: none,
  dy: dy,
)[#content]
...
#let article(
  ...
  codly(
    ...
    display-icon: true,              // Show language icons (succinct)
    display-name: false,             // Hide language names (use icon only)
  ...
  let tocblock() = context {
    ...
    [
      ...
      #text(font: serif-fonts, size: margin-size, weight: "regular", style: "italic")[  // \large
        // Filter margin notes from headings in the outline
        #show heading: it => {
          // Strip marginalia from heading body for outline display only
          let clean_body = if type(it.body) == "sequence" {
            it.body.children.filter(child => 
              not repr(child).contains("marginalia")
            ).join()
          } else {
            it.body
          }
          heading(level: it.level, numbering: it.numbering, clean_body)
        }
...
<option-1-global-user-level-installation>
This installs the toolkit once to your user Python environment, making it available from any directory.#marginalia.note(numbering: none)[#set par(first-line-indent: 0em)
#strong[Default Locations]

- Windows:
  #raw(lang:"powershell", "%USERPROFILE%\\wavelet_prosody_toolkit")
- macOS/Linux: #raw(lang:"bash", "~/wavelet_prosody_toolkit")]
...
== Usage
<usage-1>
=== Windows#marginalia.note(numbering: none)[#set par(first-line-indent: 0em)
#strong[Virtual Environment]

The toolkit is installed in an isolated environment (`.venv`) within
...
== All Platforms#marginalia.note(numbering: none)[#set par(first-line-indent: 0em)
#strong[Python Version]

Python 3.8 or later is recommended. The toolkit may work with earlier
...
#marginalia.note(numbering: none)[#set par(first-line-indent: 0em)
#strong[Dependencies]
...
#marginalia.note(numbering: none)[#set par(first-line-indent: 0em)
#strong[File Distribution]

All files are included in the distribution package and can be used
independently.]
= Additional Resources#marginalia.note(numbering: none)[#set par(first-line-indent: 0em)
#strong[Support]

For issues specific to the toolkit itself (not installation), please
refer to the GitHub repository's issue tracker.]

...
function Note(note)
    ...
    if block.t == "Para" or block.t == "Plain" then
      ...
      for _, inline in ipairs(block.content) do
        if inline.t == "RawInline" and inline.format == "typst" then
          -- Preserve raw Typst inlines (like sidecite calls from Cite function)
          table.insert(inline_parts, inline.text)
  ...
  -- Normalize quotes in content for Typst compatibility
  ...
  function Pandoc(doc)
    ...
    while i <= #blocks do
      ...
      if current.t == "Div" and current.classes:includes('column-margin') then
        ...
        -- Convert blocks to simple text string WITHOUT calling pandoc.write()
        -- This avoids triggering Quarto's filter chain which causes errors with R output
        local content_parts = {}
        for _, block in ipairs(processed_content) do
          if block.t == "Para" or block.t == "Plain" then
            for _, inline in ipairs(block.content) do
              if inline.t == "RawInline" and inline.format == "typst" then
                table.insert(content_parts, inline.text)
              elseif inline.t == "Str" then
                table.insert(content_parts, inline.text)
              elseif inline.t == "Space" then
                table.insert(content_parts, " ")
              else
                table.insert(content_parts, pandoc.utils.stringify(inline))
              end
            end
          else
            table.insert(content_parts, pandoc.utils.stringify(block))
        ...
        local content_typst = table.concat(content_parts, "")
        ...
        local margin_note_code = string.format('#sidenote(%s)[#set par(first-line-indent: 0em)\n%s]',
        ...
        if #new_blocks > 0 then
          ...
          elseif prev.t == "BulletList" or prev.t == "OrderedList" then
            ...
            -- For headers and other blocks, create standalone Plain block
...
function Header(header)
  ...
  -- Build heading content as inline Typst code
  local content_parts = {}
  for _, inline in ipairs(header.content) do
    if inline.t == "Str" then
      table.insert(content_parts, inline.text)
    ...
    elseif inline.t == "RawInline" and inline.format == "typst" then
      ...
      -- For other inline types, try to stringify
      table.insert(content_parts, pandoc.utils.stringify(inline))
    end
  end
  local content_code = table.concat(content_parts, "")

  -- Use content block syntax to wrap function calls
  -- This allows the heading to render while keeping outlined: false
...
}#heading(level: %d, outlined: false)[%s%s]]]=], 
    header.level, clean_text,
    header.level, content_code, label_code)

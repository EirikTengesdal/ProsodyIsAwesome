...
#let article(
  ...
  // Use relative path from document to extension directory
  ...
  // Exclude sidenote/footnote anchors (internal location links)
  show link: it => {
    if type(it.dest) == str {
      // External links (URLs, mailto:, etc.) - apply styling
      text(
        font: mono-fonts,
        size: 0.8em,
      ...
      // Internal links (sidenote anchors, refs) - no styling
      it
    }
  }
 ...
 //   side: right,
  ...
  if toc !=none [#marginalia.note(numbering: none, dy: 0.35em)[#tocblock()]]//[#margin-note(dx: 0em, dy: -1em)[#tocblock()]]

  // Heading spacing and formatting

  set heading(
    numbering: sectionnumbering
  )

  // Level 1 headings (numbered and unnumbered)
  ...
  // Level 2 headings (numbered and unnumbered)
  ...
  // Level 3 headings (numbered and unnumbered)
  show heading.where(level:3): it => {
    v(1em, weak:true)
  ...
  // Suppress headings beyond level 3
  show heading: it => {
...
You can download the installation scripts from #link("https://github.com/EirikTengesdal/ProsodyIsAwesome")[github.com/EirikTengesdal/ProsodyIsAwesome];.

== Support
<support>
For installation issues, please email Eirik Tengesdal at #link("mailto:eirik.tengesdal@oslomet.no")[eirik.tengesdal\@oslomet.no] with sufficient details about your system and the problem encountered. You can also open an issue on the #link("https://github.com/EirikTengesdal/ProsodyIsAwesome/issues")[GitHub repository] if you believe it is a bug.
...
}#heading(level: 1, outlined: false)[Which Installation Method?#sidenote(alignment: "baseline", dy: 0pt)[For most users, the #strong[global installation] is simpler and more convenient. Use per-project only when you need strict dependency isolation.]] <which-installation-method1>
== Global Installation (recommended):
<global-installation-recommended>
- Windows: #link("https://github.com/EirikTengesdal/ProsodyIsAwesome/blob/main/install_toolkit_standalone.ps1")[#raw(lang:"powershell", "install_toolkit_standalone.ps1");]
- macOS/Linux: #link("https://github.com/EirikTengesdal/ProsodyIsAwesome/blob/main/install_toolkit_standalone.sh")[#raw(lang:"bash", "install_toolkit_standalone.sh");]
- Use across multiple projects without reinstalling

== Per-Project Installation:
<per-project-installation>
- Windows: #link("https://github.com/EirikTengesdal/ProsodyIsAwesome/blob/main/install_toolkit_project.ps1")[#raw(lang:"powershell", "install_toolkit_project.ps1");]
- macOS/Linux: #link("https://github.com/EirikTengesdal/ProsodyIsAwesome/blob/main/install_toolkit_project.sh")[#raw(lang:"bash", "install_toolkit_project.sh");]
- Isolated dependencies and strict reproducibility

#{
  show heading: none
  heading(level: 1, outlined: true)[Option 1: Global User-Level Installation]
  counter(heading).update((..nums) => {
    let arr = nums.pos()
    arr.slice(0, -1) + (arr.last() - 1,)
  })
}#heading(level: 1, outlined: false)[Option 1: Global User-Level Installation#sidenote(numbering: none, anchor-numbering: none)[#set par(first-line-indent: 0em); #strong[Default Locations:]

- Windows: #raw(lang:"powershell", "%USERPROFILE%\\wavelet_prosody_toolkit")
- macOS/Linux: #raw(lang:"bash", "~/wavelet_prosody_toolkit")]] <option-1-global-user-level-installation>
...
}#heading(level: 1, outlined: false)[Option 2: Per-Project Installation#sidenote(alignment: "baseline", dy: 0pt)[Use this method when you need isolated dependencies for reproducible research environments.]] <option-2-per-project-installation2>
...
#{
  show heading: none
  heading(level: 3, outlined: true)[Windows]
  counter(heading).update((..nums) => {
    let arr = nums.pos()
    arr.slice(0, -1) + (arr.last() - 1,)
  })
}#heading(level: 3, outlined: false)[Windows#sidenote(numbering: none, anchor-numbering: none)[#set par(first-line-indent: 0em); #strong[Virtual Environment]

The toolkit is installed in an isolated environment (`.venv`) within your project directory.]] <windows-2>
```powershell
...
#{
  show heading: none
  heading(level: 2, outlined: true)[All Platforms]
  counter(heading).update((..nums) => {
    let arr = nums.pos()
    arr.slice(0, -1) + (arr.last() - 1,)
  })
}#heading(level: 2, outlined: false)[All Platforms#sidenote(numbering: none, anchor-numbering: none)[#set par(first-line-indent: 0em); #strong[Python Version:]

Python 3.8 or later is recommended. The toolkit may work with earlier versions but has not been tested.]] <all-platforms>
#{
  show heading: none
  heading(level: 3, outlined: true)[Python 3]
  counter(heading).update((..nums) => {
    let arr = nums.pos()
    arr.slice(0, -1) + (arr.last() - 1,)
  })
}#heading(level: 3, outlined: false)[Python 3#sidenote(numbering: none, anchor-numbering: none)[#set par(first-line-indent: 0em); Download from: #link("https://www.python.org/downloads/")[python.org/downloads]]] <python-3>
#strong[Windows:]
...
#{
  show heading: none
  heading(level: 3, outlined: true)[Git]
  counter(heading).update((..nums) => {
    let arr = nums.pos()
    arr.slice(0, -1) + (arr.last() - 1,)
  })
}#heading(level: 3, outlined: false)[Git#sidenote(numbering: none, anchor-numbering: none)[#set par(first-line-indent: 0em); Download from: #link("https://git-scm.com/download/win")[git-scm.com/download/win]]] <git>
#strong[Windows:]
...
"@ | Out-File -Encoding utf8 analyse.py
...
#table(
  ...
  table.hline(),
  [Global installation], [#link("https://github.com/EirikTengesdal/ProsodyIsAwesome/blob/main/install_toolkit_standalone.ps1")[#raw(lang:"powershell", "install_toolkit_standalone.ps1");];], [#link("https://github.com/EirikTengesdal/ProsodyIsAwesome/blob/main/install_toolkit_standalone.sh")[#raw(lang:"bash", "install_toolkit_standalone.sh");];],
  [Per-project installation], [#link("https://github.com/EirikTengesdal/ProsodyIsAwesome/blob/main/install_toolkit_project.ps1")[#raw(lang:"powershell", "install_toolkit_project.ps1");];], [#link("https://github.com/EirikTengesdal/ProsodyIsAwesome/blob/main/install_toolkit_project.sh")[#raw(lang:"bash", "install_toolkit_project.sh");];],
)
]
]
#sidenote(numbering: none, anchor-numbering: none, dy: 1em, shift: "ignore")[#set par(first-line-indent: 0em); #strong[File Distribution:]
